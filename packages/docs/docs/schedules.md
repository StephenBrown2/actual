...
From the Schedules page, click **Import / Export** to open a hub where you choose a flow:

- **Export to JSON** — opens a short export dialog; confirming runs your system’s **Save** dialog so you pick the folder and filename (default name includes the budget name and date). Actual does not show the full path in the app afterward.
- **Import from JSON** — opens a review dialog: pick a `.json5` or `.json` schedules file, see a list of schedules in the file, select which rows to import, then import. Only the schedules you select are applied.
- **Import from Wallos** — import subscriptions from [Wallos](https://github.com/ellite/Wallos) as schedules (see also [community projects](/docs/community-repos.md)).
