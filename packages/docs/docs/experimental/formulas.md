...
| `BALANCE_OF`                    | Query, Rules | One argument (Rules): running balance for another account in cents, same cutoff as `balance`. Two arguments: balance for that account as of end of day on the resolved date (inclusive). | `=BALANCE_OF("Checking"); =BALANCE_OF("Checking", "2026-01-15"); =BALANCE_OF("Checking", EDATE(DATEVALUE(today),-1))`         | Second arg: `dateExpr` (optional in Rules for one-arg form) |
| `BALANCE_ON`                    | Rules        | Balance of the current transaction’s account as of end of day on the resolved date (inclusive), in cents. Same date semantics as two-arg `BALANCE_OF`.                                   | `=BALANCE_ON("2026-01-15"); =BALANCE_ON(date)`                                                                                | —                                                           |
...
### Account balance: `BALANCE_OF` on Formula cards

On **Formula cards** (query mode), use the **two-argument** form `BALANCE_OF("accountIdOrName", dateExpr)` to get an account balance as of a calendar date (inclusive). The second argument is evaluated like the rest of the formula (for example a quoted calendar date as **`"DD/MM/YYYY"`** — HyperFormula’s default text form — or **`"YYYY-MM-DD"`** (ISO), or a function such as `EDATE` / `DATEVALUE` / `today`). One-argument `BALANCE_OF("…")` is intended for **Rules** where a “current transaction” exists; on Formula cards prefer the two-argument form. `BALANCE_ON` is **not** available on Formula cards (Rules only).
...
=BALANCE_OF("Checking")
=BALANCE_OF("Checking", "2026-01-15")
=BALANCE_OF("Checking", date)
```
...
- `BALANCE_OF("…", dateExpr)` — Balance for that account, in **cents**, as of **end of day** on the date that `dateExpr` evaluates to (inclusive of all transactions on that day). The sub-expression may resolve to a **number** (Excel date serial), or a **string** in **`YYYY-MM-DD`** (ISO) or **`DD/MM/YYYY`** (HyperFormula’s default date-string form, day/month may be one or two digits). Use a quoted literal, the `date` variable (ISO from Actual), or `DATE` / `TEXT` / `EDATE` / `DATEVALUE` (see **21st of previous month** below). To parse the rule variable `date` with `DATEVALUE`, first turn it into a DD/MM/YYYY string (for example `MID(date,9,2)&"/"&MID(date,6,2)&"/"&MID(date,1,4)`); `DATEVALUE` does not accept the raw ISO form in HyperFormula.
...
- If the account is not found, the value is **0**. If the date expression is invalid, the value is **0**.
- For the **current** transaction’s account at the same cutoff as `balance`, use the `balance` variable instead of one-arg `BALANCE_OF`.
- Returns an integer in cents. For the amount in dollars, divide by 100 or `=INTEGER_TO_AMOUNT(BALANCE_OF("Checking"))`.

#### Example: balance as of the 21st of the **previous** calendar month

Use this when a statement closes on a fixed day (here **21**) in the month **before** the transaction’s month (e.g. March activity → cutoff **February 21**).

The `date` rule variable is the transaction date as `YYYY-MM-DD`. Subtract one month with `EDATE(..., -1)`, then build day **21** in that year/month.

HyperFormula’s `DATEVALUE` expects a **DD/MM/YYYY**-style string, not the raw ISO `date` string. Build a DD/MM/YYYY string with `MID`, pass it to `DATEVALUE` for a serial, then use `EDATE` and `TEXT(..., "dd/mm/yyyy")`. Actual accepts either **ISO** or **DD/MM/YYYY** strings from the sub-expression:

```text
=BALANCE_OF("My credit card", TEXT(DATE(YEAR(EDATE(DATEVALUE(MID(date,9,2)&"/"&MID(date,6,2)&"/"&MID(date,1,4)), -1)), MONTH(EDATE(DATEVALUE(MID(date,9,2)&"/"&MID(date,6,2)&"/"&MID(date,1,4)), -1)), 21), "dd/mm/yyyy"))
```

Equivalent using `DATE` + `VALUE` + `MID` (no `DATEVALUE`) and ISO text output — still supported:

```text
=BALANCE_OF("My credit card", TEXT(DATE(YEAR(EDATE(DATE(VALUE(MID(date,1,4)),VALUE(MID(date,6,2)),VALUE(MID(date,9,2))), -1)), MONTH(EDATE(DATE(VALUE(MID(date,1,4)),VALUE(MID(date,6,2)),VALUE(MID(date,9,2))), -1)), 21), "yyyy-mm-dd"))
```

- `DATEVALUE(MID(date,9,2)&"/"&MID(date,6,2)&"/"&MID(date,1,4))` turns Actual’s ISO `date` into a serial via a DD/MM/YYYY string.
- `EDATE(..., -1)` moves one month backward; `YEAR` / `MONTH` of that anchor the **previous calendar month**, and the day `21` is your statement closing day.
- For a **different** closing day, replace each `21` with your statement day (1–31).

**`BALANCE_ON` (current account only, Rules):**

```text
=BALANCE_ON("2026-01-15")
=BALANCE_ON(date)
```

- Same end-of-day inclusive semantics as `BALANCE_OF("…", dateExpr)`, but for **this** transaction’s account (no account argument). Not available in Formula cards.

#### Example with date calculation for previous month

```text
=BALANCE_ON(TEXT(DATE(YEAR(EDATE(DATEVALUE(MID(date,9,2)&"/"&MID(date,6,2)&"/"&MID(date,1,4)), -1)), MONTH(EDATE(DATEVALUE(MID(date,9,2)&"/"&MID(date,6,2)&"/"&MID(date,1,4)), -1)), 21), "dd/mm/yyyy"))
```
