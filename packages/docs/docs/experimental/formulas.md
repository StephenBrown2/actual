...
| `INTEGER_TO_AMOUNT`             | Query, Rules | Converts integer amount to decimal amount (e.g., 1234 -> 12.34).                                                                                               | `=INTEGER_TO_AMOUNT(amount, 2)`                                                                                               | `decimal_places` (optional; budget currency minor units, else 2) |
...
- Returns an integer in cents. For the amount in dollars, divide by 100 or `=INTEGER_TO_AMOUNT(BALANCE_OF("Checking"))` (uses the budget currency’s minor units, or you can pass a second argument such as `2`).
