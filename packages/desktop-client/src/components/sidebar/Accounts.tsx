...
import { groupAccountsBySubgroup } from '#accounts/accountSubgroups';
...
export function Accounts() {
  ...
  const treeItems: TreeNode[] = useMemo(() => {
    const treeItemChildren: TreeNode[] = [];

    if (onBudgetAccounts.length > 0) {
      treeItemChildren.push({
        id: ON_BUDGET_KEY,
        name: t('On budget'),
  ...
  const allSubgroupKeys = useMemo(() => {
    const keys: string[] = [];
    function walkSubgroups(nodes: TreeNode[]) {
      for (const node of nodes) {
        if (node.isSubgroup) {
          keys.push(node.id);
        }
        if (node.children) {
          walkSubgroups(node.children);
        }
      }
    }
    walkSubgroups(treeItems);
    return keys;
  }, [treeItems]);

  const expandedKeys: Set<Key> = useMemo(() => {
    if (savedExpandedKeys) {
      ...
      return new Set<Key>([
        ...expanded,
        ...allSubgroupKeys.filter(key => !seen.has(key)),
      ]);
    }
    return new Set<Key>([...STRUCTURAL_EXPANDED_KEYS, ...allSubgroupKeys]);
  }, [savedExpandedKeys, allSubgroupKeys]);
  ...
  function persistExpandedKeys(keys: Set<Key>) {
    expandedKeysRef.current = new Set(keys);
    const seenMarkers = allSubgroupKeys.map(key => `${SEEN_KEY_PREFIX}${key}`);
  ...
  function getAccountContainerKey(accountId: string): string | null {
  ...
  function getSubgroupKeysForPrefix(prefix: string): string[] {
    const budgetNode = treeItems[0]?.children?.find(c => c.id === prefix);
    return budgetNode?.children?.filter(c => c.isSubgroup).map(c => c.id) ?? [];
  }

  function getAccountSiblingIdsForTarget(targetAccountId: string): string[] {
    const rootChildren = treeItems[0]?.children ?? [];
    for (const group of rootChildren) {
      const groupChildren = group.children ?? [];
                ...
                <TreeItem key={node.id} id={node.id} textValue={node.name}>
                  <TreeItemContent>
                    <Account
                      ...
                      failed={isAccountFailedSync(node.account)}
