...
> {
  ...
  const { payee, account, amount: amountCond, date: dateCond } =
  ...
  const payeeStr = payee
    ? conditionScalarToDisplay(
        (payee as { value?: unknown }).value,
      )
    : null;
  const accountStr = account
    ? conditionScalarToDisplay(
        (account as { value?: unknown }).value,
      )
    : null;
...
function buildRows(
  ...
  return payload.schedules.map((item, index) => {
    ...
    const isDuplicate =
      normalized != null && existingNames.has(normalized);
    ...
    ({ item }: { item: JsonScheduleRow }) => {
      ...
      return (
        ...
        >
          ...
          />
          <Field width="flex" name="name" truncate={false} style={{ minWidth: 0 }}>
          ...
          <Field width="flex" name="payee" truncate={false} style={{ minWidth: 0 }}>
            <Text
              style={{
                overflow: 'hidden',
                textOverflow: 'ellipsis',
          ...
          <Field width="flex" name="account" truncate={false} style={{ minWidth: 0 }}>
            <Text
              style={{
                overflow: 'hidden',
                textOverflow: 'ellipsis',
          ...
          <Field width={110} name="date" truncate={false}>
            {item.nextDate
              ? monthUtilFormat(item.nextDate, dateFormat)
              : null}
          </Field>
          {item.amount != null && item.amountOp != null ? (
            <ScheduleAmountCell amount={item.amount} op={item.amountOp} />
          ) : (
            <Field width={100} truncate={false} style={{ textAlign: 'right' }} />
          )}
