...
function parseScheduleTransfer(raw: string): ScheduleTransferPayload {
  ...
  try {
    ...
    throw new Error('Unable to parse schedules file as JSON');
  }
    ...
    schedules: schedules.map(schedule => {
      ...
      const actionsSource = Array.isArray(serializedRule?.actions)
        ...
        : Array.isArray(schedule._actions)
          ? schedule._actions
          : [];
      const conditions = mapRuleEntriesForExport(cloneJSON(conditionsSource), {
        accountNamesById,
        payeeNamesById,
        categoryNamesById,
        categoryGroupNamesById,
...
function pickImportedScheduleName(
  item: ScheduleTransferItem,
  sourceIndex: number,
  scheduleNamesByIndex?: Record<number, string>,
): string | null {
  const override = scheduleNamesByIndex?.[sourceIndex];
...
export async function importSchedules({
  ...
  /** When set, use these names (by source index in the file) instead of each item's `name`. */
  scheduleNamesByIndex?: Record<number, string>;
}): Promise<ScheduleImportResult> {
  const payload = parseScheduleTransfer(content);
  const allSchedules = payload.schedules;
  const indicesToImport =
    scheduleIndices != null && scheduleIndices.length > 0
      ? [...new Set(scheduleIndices)].filter(
          idx => Number.isInteger(idx) && idx >= 0 && idx < allSchedules.length,
        )
      : allSchedules.map((_, i) => i);
  ...
  for (const idx of indicesToImport) {
    const scheduleItem = allSchedules[idx];
    let importedScheduleId: string | null = null;
    const scheduleNameForErrors = pickImportedScheduleName(
      scheduleItem,
      idx,
      scheduleNamesByIndex,
    );
    try {
      ...
      const importedScheduleName = pickImportedScheduleName(
        scheduleItem,
        idx,
        scheduleNamesByIndex,
      );
      ...
      importedScheduleId = await createSchedule({
        ...
        conditions: mappedConditions as RuleConditionEntity[],
      });
      ...
      result.skipped++;
      result.errors.push({
        scheduleName: scheduleNameForErrors,
...
async function exportSchedulesJson() {
  try {
    const content = await exportSchedules();
    return { data: Buffer.from(content, 'utf-8') };
  } catch (err) {
    const e = err instanceof Error ? err : new Error(String(err));
    e.message = 'Error exporting schedules: ' + e.message;
    captureException(e);
    return { error: 'internal-error' };
  }
}

async function parseImportSchedulesJson({
  filepath,
}: {
  filepath: string;
}): Promise<ScheduleTransferPayload | { error: string }> {
  try {
    if (!(await fs.exists(filepath))) {
      throw new Error(`File not found at the provided path: ${filepath}`);
    }
    const raw = await fs.readFile(filepath);
    return parseScheduleTransfer(raw);
  } catch (err) {
    const e = err instanceof Error ? err : new Error(String(err));
    captureException(e);
...
async function importSchedulesJson(arg: {
  filepath?: string;
  content?: string;
  scheduleIndices?: number[];
  scheduleNamesByIndex?: Record<number, string>;
}): Promise<ScheduleImportResult | { error: string }> {
  try {
    let raw: string;
    if (arg.filepath) {
      if (!(await fs.exists(arg.filepath))) {
        throw new Error(`File not found at the provided path: ${arg.filepath}`);
      }
      raw = await fs.readFile(arg.filepath);
    } else if (arg.content != null) {
      raw = arg.content;
    } else {
      throw new Error('importSchedulesJson requires filepath or content');
    }
    return await importSchedules({
      content: raw,
      scheduleIndices: arg.scheduleIndices,
      scheduleNamesByIndex: arg.scheduleNamesByIndex,
    });
  } catch (err) {
    const e = err instanceof Error ? err : new Error(String(err));
    e.message = 'Error importing schedules: ' + e.message;
    captureException(e);
    return { error: 'internal-error' };
  }
}
